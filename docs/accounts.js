...
const accountsModule = {
  ...
  mutations: {
    ...
    async updateCollection(state, { syncMode, filterUpdate }) {
      ...
      async function processSales(data) {
        ...
        //     // if (count == 0) {
        //     //   logInfo("accountsModule", "mutations.loadSales().processSales() " + new Date(sale.timestamp * 1000).toLocaleString() + " " + (sale.token.name ? sale.token.name : "(null)") + ", price: " + sale.price + ", from: " + sale.from.substr(0, 10) + ", to: " + sale.to.substr(0, 10));
        //     // }
      ...
      logInfo("accountsModule", "mutations.updateCollection() - syncMode: " + syncMode + ", filterUpdate: " + JSON.stringify(filterUpdate));
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const block = await provider.getBlock("latest");
        const blockNumber = block.number;
        ...
        // logInfo("accountsModule", "mutations.updateCollection() - url: " + url);
        ...
        if (false) {
          ...
          //     (continuation != null ? "&continuation=" + continuation : '');
          //   // logInfo("accountsModule", "mutations.loadSales() - url: " + url);
          //   // logInfo("accountsModule", "mutations.loadSales() - Retrieving records for " + new Date(processFrom).toLocaleString() + " to " + new Date(processTo).toLocaleString());
          //   const data = await fetch(url)
          //     .then(handleErrors)
          //     .then(response => response.json())
          //     .catch(function(error) {
    ...
    async monitorMints(state, { syncMode, filterUpdate }) {
      // --- monitorMints() start ---
      logInfo("accountsModule", "mutations.monitorMints() - syncMode: " + syncMode + ", filterUpdate: " + JSON.stringify(filterUpdate));
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const block = await provider.getBlock("latest");
        const blockNumber = block.number;
  ...
  actions: {
    ...
    updateCollection(context, { syncMode, filterUpdate }) {
      logInfo("accountsModule", "actions.updateCollection() - syncMode: " + syncMode + ", filterUpdate: " + JSON.stringify(filterUpdate));
      context.commit('updateCollection', { syncMode, filterUpdate });
    },
    monitorMints(context, { syncMode, filterUpdate }) {
      logInfo("accountsModule", "actions.monitorMints() - syncMode: " + syncMode + ", filterUpdate: " + JSON.stringify(filterUpdate));
