const NFTs = {
  ...
  mounted() {
    logInfo("NFTs", "mounted() $route: " + JSON.stringify(this.$route.params));
...
const nftsModule = {
  ...
  mutations: {
    ...
    async addToExecutionQueue(state, action) {
      logInfo("nftsModule", "mutations.addToExecutionQueue() - executionQueue: " + JSON.stringify(state.executionQueue));
      // TODO - Not working
      if (!state.executionQueue.includes(action)) {
        state.executionQueue.push(action);
      } else {
        logInfo("nftsModule", "mutations.addToExecutionQueue() - already existing action: " + JSON.stringify(action) + ", queue: " + JSON.stringify(state.executionQueue));
      }
      logInfo("nftsModule", "mutations.addToExecutionQueue() - action: " + JSON.stringify(action) + ", queue: " + JSON.stringify(state.executionQueue));
    },
    // --- loadSales() ---
    async loadSales(state, fullSync) {
      ...
      async function updateDBFromAPI() {
        ...
        // logInfo("nftsModule", "mutations.loadSales() - db: " + (earliestDate ? earliestDate.toLocaleString() : '(null)') + " to " + (latestDate ? latestDate.toLocaleString() : '') + " @ " + now.toLocaleString());
        ...
        // logInfo("nftsModule", "mutations.loadSales() - segmentStart: " + segmentStart.toLocaleString() + ", retrieveLastDate: " + retrieveLastDate.toLocaleString() + ", deleteBeforeDate: " + deleteBeforeDate.toLocaleString());
        ...
        while (to > retrieveLastDate && !state.halt && count < 2) {
          ...
          if (!(from.toLocaleString() in dates)) {
            ...
            if (totalRecords > 0) {
              logInfo("nftsModule", "mutations.loadSales() - Retrieved " +  totalRecords + " record(s) for " + new Date(processFrom).toLocaleString() + " to " + new Date(processTo).toLocaleString());
            } else {
      ...
      logInfo("nftsModule", "mutations.loadSales() - fullSync: " + fullSync);
  ...
  actions: {
    ...
    //   logInfo("nftsModule", "actions.execWeb3() - count: " + count);
    //   commit('doit', { action: "refresh" } );
    // },
    updateFilter(context, action) {
      logInfo("nftsModule", "actions.updateFilter() - action: " + JSON.stringify(action));
      // context.commit('addToExecutionQueue', action);
    },
    loadSales(context, fullSync) {
      // logInfo("nftsModule", "actions.loadSales() - fullSync: " + fullSync);
      context.commit('loadSales', fullSync);
    },
    halt(context) {
      // logInfo("nftsModule", "actions.halt()");
      context.commit('halt');
    },
  },
};
