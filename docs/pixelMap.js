const PixelMap = {
  ...
  computed: {
    ...
    config() {
      return store.getters['pixelMap/config'];
    },
    filter() {
      return store.getters['pixelMap/filter'];
    },
    sync() {
      return store.getters['pixelMap/sync'];
    },
    collectionInfo() {
      return store.getters['pixelMap/collectionInfo'];
    },
    collectionTokens() {
      return store.getters['pixelMap/collectionTokens'];
    },
    ensMap() {
      return store.getters['pixelMap/ensMap'];
    },
  ...
  methods: {
    ...
    updateURL(where) {
      this.$router.push('/pixelMap/' + where);
    },
    async updateCollection(syncMode, filterUpdate) {
      store.dispatch('pixelMap/updateCollection', { syncMode, filterUpdate });
    },
    async halt() {
      store.dispatch('pixelMap/halt');
    },
    async timeoutCallback() {
      logDebug("PixelMap", "timeoutCallback() count: " + this.count);
      this.count++;
      var t = this;
      if (this.reschedule) {
        setTimeout(function() {
  ...
  beforeDestroy() {
    logDebug("PixelMap", "beforeDestroy()");
  },
  mounted() {
    logInfo("PixelMap", "mounted() $route: " + JSON.stringify(this.$route.params) + ", props['tab']: " + this.tab + ", props['blocks']: " + this.blocks + ", props['search']: " + this.search);
    if (this.tab == "transfers") {
      ...
      if (this.blocks != null) {
        ...
        setTimeout(function() {
          store.dispatch('pixelMap/monitorMints', { syncMode: 'scan', filterUpdate });
        }, 1000);
      }
    }
    this.reschedule = true;
    logDebug("PixelMap", "Calling timeoutCallback()");
...
const pixelMapModule = {
  ...
  mutations: {
    ...
    async updateCollection(state, { syncMode, filterUpdate }) {
      ...
      if (window.ethereum) {
        ...
        state.sync.total = scanTo - scanFrom;
        state.sync.completed = 0;
  ...
  actions: {
    updateCollection(context, { syncMode, filterUpdate }) {
      logInfo("pixelMapModule", "actions.updateCollection() - syncMode: " + syncMode + ", filterUpdate: " + JSON.stringify(filterUpdate));
