const Transactions = {
  ...
  mounted() {
    logInfo("Transactions", "mounted() $route: " + JSON.stringify(this.$route.params));
...
const transactionsModule = {
  ...
  mutations: {
    async search(state, { searchType, searchString, searchGroup } ) {
      ...
      if (searchType == 'name' || searchType == 'owner') {
        ...
        searchForAccounts = searchString == null ? [] : searchString.split(/[, \t\n]+/)
          .map(function(name) { return name.toLowerCase().trim(); })
          .filter(function (name) { return name.length == 42 && name.substring(0, 2) == '0x'; });
        // logInfo("transactionsModule", "mutations.search() - searchString: " + searchString);
        // logInfo("transactionsModule", "mutations.search() - searchForNames: " + JSON.stringify(searchForNames, null, 2));
        // logInfo("transactionsModule", "mutations.search() - searchForAccounts: " + JSON.stringify(searchForAccounts, null, 2));
        ...
        // logInfo("transactionsModule", "mutations.search() - data: " + JSON.stringify(data, null, 2));
        const registrations = data.data.registrations || [];
        const registrantMap = {};
        // logInfo("transactionsModule", "mutations.search() - registrations: " + JSON.stringify(registrations, null, 2));
        for (registration of registrations) {
          // logInfo("transactionsModule", "mutations.search() - registration: " + JSON.stringify(registration, null, 2));
          if (searchType == 'owner') {
        ...
        // logInfo("transactionsModule", "mutations.search() - results: " + JSON.stringify(Object.keys(results), null, 2));
        const namesFound = Object.keys(results).map(function(name) { return name.replace('.eth', ''); });
        // logInfo("transactionsModule", "mutations.search() - namesFound: " + JSON.stringify(namesFound, null, 2));
        state.notRegistered = searchForNames.filter(name => !namesFound.includes(name));
        // logInfo("transactionsModule", "mutations.search() - notRegistered: " + JSON.stringify(state.notRegistered, null, 2));
        state.notRegistered.sort(function (a, b) {
            return ('' + a).localeCompare(b);
        })
        // logInfo("transactionsModule", "mutations.search() - notRegistered sorted: " + JSON.stringify(state.notRegistered, null, 2));
        // logInfo("transactionsModule", "mutations.search() - registrantMap sorted: " + JSON.stringify(registrantMap, null, 2));
      ...
      if (searchType == 'owner' || searchType == 'group') {
        ...
        // logInfo("transactionsModule", "mutations.search() - searchForAccounts: " + JSON.stringify(searchForAccounts));
        // logInfo("transactionsModule", "mutations.search() - expiryDate: " + expiryDate + " = " + new Date(expiryDate * 1000));
        state.message = "Retrieving";
        for (account of searchForAccounts) {
          // logInfo("transactionsModule", "mutations.search() - account: " + JSON.stringify(account));
      ...
      // logInfo("transactionsModule", "mutations.search() - results: " + JSON.stringify(results, null, 2));
    ...
    async scanDigits(state, { selectedDigit, scanFrom, scanTo, length, digitPrefix, digitPostfix } ) {
      ...
      logInfo("transactionsModule", "mutations.scanDigits(): " + selectedDigit + ", " + scanFrom + ", " + scanTo + ", " + length + ", " + digitPrefix + ", " + digitPostfix);
      const results = {};
      const now = parseInt(new Date().valueOf() / 1000);
      const expiryDate = parseInt(now) - 90 * 24 * 60 * 60;
      const warningDate = parseInt(now) + 90 * 24 * 60 * 60;
      ...
      for (let iBatch = scanFrom; iBatch < scanTo && !state.stopScan; iBatch = parseInt(iBatch) + ENSSUBGRAPHBATCHSCANSIZE) {
        // logInfo("transactionsModule", "mutations.scanDigits() - iBatch: " + iBatch);
        const max = (parseInt(iBatch) + ENSSUBGRAPHBATCHSCANSIZE - 1) < scanTo ? parseInt(iBatch) + ENSSUBGRAPHBATCHSCANSIZE - 1: scanTo;
        // logInfo("transactionsModule", "mutations.scanDigits() - from: " + iBatch + " to " + max);
        const numbers = generateRangeZeroPad(iBatch, max, 1, length, digitPrefix, digitPostfix);
        // logInfo("transactionsModule", "mutations.scanDigits() - numbers: " + numbers);
        ...
        const namesFound = Object.keys(results).map(function(name) { return name.replace('.eth', ''); });
        // logInfo("transactionsModule", "mutations.scanDigits() - namesFound: " + JSON.stringify(namesFound, null, 2));
        const notRegistered = numbers.filter(name => !namesFound.includes(name));
        // logInfo("transactionsModule", "mutations.scanDigits() - notRegistered: " + JSON.stringify(notRegistered, null, 2));
        state.notRegistered.push(...notRegistered);
        // logInfo("transactionsModule", "mutations.scanDigits() - all notRegistered: " + JSON.stringify(state.notRegistered, null, 2));
      }
      state.results = results;
      state.message = null;
      state.stopScan = false;
  ...
  actions: {
    search(context, { searchType, searchString, searchGroup } ) {
      // logInfo("transactionsModule", "actions.search(): " + searchType + ", " + searchString + ", " + searchGroup);
    ...
    scanDigits(context, { selectedDigit, scanFrom, scanTo, length, digitPrefix, digitPostfix } ) {
      logInfo("transactionsModule", "actions.scanDigits(): " + selectedDigit + ", " + scanFrom + ", " + scanTo + ", " + length + ", " + digitPrefix + ", " + digitPostfix);
      context.commit('scanDigits', { selectedDigit, scanFrom, scanTo, length, digitPrefix, digitPostfix } );
    },
    stopScan(context) {
      logInfo("transactionsModule", "actions.stopScan()");
